...
import { dieuHuongTheoVaiTro, dieuHuongMobile } from "@/lib/nav-config";
...
function NavLinks({ pathname, items, onNavigate }: {
  ...
  const daLoc = items.filter(m => m.nhan.toLocaleLowerCase("vi").includes(tim.toLocaleLowerCase("vi").trim()));
  return <div className="space-y-5">
    ...
    <nav aria-label="Menu chính" className="space-y-1">
      ...
      {daLoc.length === 0 && <p className="px-3 text-sm text-muted-foreground">Không tìm thấy chức năng.</p>}
    </nav>
  </div>;
}
