  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
        <StatCard nhan="Khách hàng mới (tháng)" giaTri={String(soKhachMoi?.count ?? 0)} />
        <StatCard nhan="Đơn tạo mới (tháng)" giaTri={String(d.don_moi_thang)} />
        <StatCard nhan="Đơn hoàn thành (tháng)" giaTri={String(d.don_hoan_thanh_thang)} />
        <StatCard nhan="Doanh thu (tháng)" giaTri={formatVND(d.doanh_thu_thang)} />
        <StatCard nhan="Đã thu (tháng)" giaTri={formatVND(d.da_thu_thang)} />
        <StatCard
        ...
        />
        <StatCard nhan="Tỷ lệ chuyển đổi báo giá" giaTri={`${d.ty_le_chuyen_doi}%`} mucCanhBao={d.ty_le_chuyen_doi >= 50 ? "xanh" : d.ty_le_chuyen_doi >= 30 ? "vang" : "do"} />
        <StatCard nhan="Tỷ lệ sửa lại" giaTri={`${d.ty_le_sua_lai}%`} mucCanhBao={d.ty_le_sua_lai <= 5 ? "xanh" : d.ty_le_sua_lai <= 15 ? "vang" : "do"} />
        <StatCard nhan="Biên lợi nhuận tạm tính" giaTri={`${d.bien_loi_nhuan}%`} mucCanhBao={d.bien_loi_nhuan >= 20 ? "xanh" : d.bien_loi_nhuan >= 10 ? "vang" : "do"} ghiChu="Xấp xỉ — chưa gồm chi phí nhân công/vận hành" />
      </div>
    </div>
  );
}
